...
public class IslandBoard {
    ...
    /**
     * Method to move a worker to a new position.
     *
     * @param worker      The instance of worker to move.
     * @param newPosition Rrturns the tile instance where to put the worker.
     */
    public void changePosition(Worker worker, Tile newPosition) {
        if (worker == null) throw new NullPointerException();
        if (newPosition == null) throw new NullPointerException();
        worker.getTile().setCurrentWorker(null);
        newPosition.setCurrentWorker(worker);
        worker.setTile(newPosition);
    }
}